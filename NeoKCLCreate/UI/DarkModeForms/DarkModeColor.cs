...
public static class ProgramColors
{
    ...
    public static void ReloadTheme(Control control)
    {
        if (control is not IReloadTheme)
        {
            ...
            else if (control is PropertyGrid PG)
            {
                PG.ForeColor = TextColor;
                PG.BackColor = WindowColor;

                PG.HelpForeColor = TextColor;
                PG.HelpBackColor = ControlBackColor;

                PG.ViewForeColor = TextColor;
                PG.ViewBackColor = WindowColor;
                PG.ViewBorderColor = BorderColor;

                PG.SelectedItemWithFocusForeColor = TextColor;
                PG.SelectedItemWithFocusBackColor = ControlBackColor;

                PG.LineColor = ControlBackColor;

                PG.Refresh(); // Unfortunately required
            }
